...
## 8. 에이전트 협업 = 파일로 조율 (여러 에이전트로 일할 때만)
- 여러 에이전트/세션이 이어서·동시에 일하면, 조율 상태를 **파일**로 공유한다(`.dev/collab/`). 대화·기억이 아니라 파일이라야 다음 세션이 이어받는다.
- 세 가지 표면: **인수인계서**(`handoff/` — 넘길 때 끝낸 것/남은 것/함정), **작업 보드**(`BOARD.md` — 누가 무엇을), **선점 락**(`claims/` — 동시 작업 충돌 방지).
- **git 분리**: 오래 남길 기록(인계서·결정)은 커밋해 공유·리뷰하고, 휘발성 라이브 상태(보드·락)는 gitignore로 빼 머지 충돌을 막는다.
- **동시 작업 = 워크트리 + 심링크**: 에이전트를 `git worktree`로 격리하면 작업 트리 충돌이 사라진다. 단, worktree는 ignore 파일을 공유하지 않으므로 gitignore된 보드·락은 primary 정본으로 **심볼릭 링크**해 한 인스턴스를 공유한다. "git이 못 나르는 파일만 링크" — 추적 파일은 git이 동기화한다.
- **단일 에이전트면 만들지 않는다.** 협업이 없으면 빈 조율 폴더는 노이즈일 뿐(원칙 7: 안 쓰면 지운다).
